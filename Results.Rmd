...
Graph1 <- Congress_TfIdf%>%
  group_by(tf_idf>0.0002833)%>%
  top_n(15, abs(tf_idf)) %>%
  mutate(Term = reorder(bigram, tf_idf)) %>%
  ggplot(aes(Term, tf_idf, fill = tf_idf>0.0002833)) +
...
ggplot(frequency, aes(R, D)) +
  ...
  labs(title="Comparing the Frequency of Terms used by Republicans 
       and Democrats 48 Hours After Mass Shootings, 2--7-17",
...
Graph2 <- Congress_before_TfIdf%>%
  group_by(tf_idf>0.0002705)%>%
  top_n(15, abs(tf_idf)) %>%
  mutate(Term = reorder(bigram, tf_idf)) %>%
  ggplot(aes(Term, tf_idf, fill = tf_idf>0.0002705)) +
  geom_col(show.legend = TRUE) +
  coord_flip() +
  labs(title="Frequency of Words Used 48 Hours 
...
regression1 <- glm(mentions_terror ~ R_or_D+Nominate_dim2, 
...
terrorism<- df2 %>%
  data_grid(R_or_D,Nominate_dim2)%>%
...
ggplot(terrorism,
       aes(Nominate_dim2, pred))+
  geom_smooth(aes(color=R_or_D))+
  scale_colour_manual(labels = c("D", "R"), 
                      values=c("blue","red"))+
  labs(title="Relationship Between Probability of Mentioning Terrorism, 
...
regression2 <- glm(Shooting_related_terrorism~R_or_D+Nominate_dim2+
                race+race*R_or_D+ race*Nominate_dim2, 
...
set.seed(1234)
df3_split <- resample_partition(df3, c(test=0.5,train=0.5))
df3_train <- as_tibble(df3_split$train)
df3_test <- as_tibble(df3_split$test)

mod_fit1 <- glm(Shooting_related_terrorism~R_or_D+Nominate_dim2+
                race+race*R_or_D+ race*Nominate_dim2, 
                data = df3_train,family= "binomial")

mod_fit2 <-glm(Shooting_related_terrorism~R_or_D+Nominate_dim2+race, 
                data = df3_train,family="binomial")

lrtest(mod_fit1, mod_fit2)

#OR
pR2(mod_fit1)

#MCFADDEN=0.95 ....>good Model
...
terrorism2<- df3 %>%
  data_grid(R_or_D,Nominate_dim2, race,.model = regression2)%>%
...
ggplot(terrorism2,
       aes(Nominate_dim2,pred, color=R_or_D))+
  geom_smooth()+
  facet_wrap(~race)+
